...
这种写法下`Account`直接作为实例的元表，如果某种行为或属性没有定义，则直接到`Account`中去查找。如果我们需要某些基类没有提供的新特性，比如某个VIP账号可以透支，那么就需要创建新的取钱方法，此时相当于基类派生出了子类，并且子类复写了基类提供的取钱方法，基类提供了默认的取钱方法，但子类可以有自己的取钱逻辑，如果需要就自己去实现，即多态。
...
根据上面的思路，如果一个类需要继承自多个父类，我们需要将`__index`变成一个可以在多个父类中搜索成员变量或函数的函数，同时，需要一个专门的函数`createClass`来创建这个有多个父类的类，
...
上面的代码中，返回的表只有三个公有的方法，对私有成员`self`进行操作，调用这些方法时，我们也不需要之前的`:`操作符。

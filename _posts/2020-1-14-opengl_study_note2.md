...
注意顶点着色器中输出的变量名要和片段着色器中输入的变量名保持一致，片段着色器最终输出的颜色是纹理图片的颜色与顶点数据中的颜色属性相乘的结果，因此最后的输出看起来会是图片纹理上盖了一层彩色色盘。sampler2D是GLSL提供的用作采样器的数据类型，用来储存纹理对象。最后用GLSL提供的`texture`方法来对纹理进行采样。

...
* `GiveAbility`：必须在有Authority的Owner上调用，传入一个GASpec，检查锁定计数是否大于0，如果是则将其保存在pending列表中，否则将传入的spec加入`ActivatableAbilities`数组中
...
* `CanActivateAbility`：检查技能释放条件，流程如下
    ...
    * 检查标签是否满足条件
    * 检查inputid是否被屏蔽
* `TryActivateAbility`：调用`CanActivateAbility`，以及各种检查，最后`CommitAbility`
